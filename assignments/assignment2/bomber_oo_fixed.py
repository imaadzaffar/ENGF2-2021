# Modified by Imaad Zaffar 15.10.21
# Fixed version of the bomber game in assignment 2
...
class Building:
    def __init__(self, canvas, building_num, height, width):
        ...
        self.width = width
        self.main_rect = canvas.create_rectangle(
            self.x,
            CANVAS_HEIGHT,
            self.x + self.width,
    ...
    def is_inside(self, point):
        if (
            point.X < self.x - 10
            or point.X > self.x + self.width
            or point.Y < CANVAS_HEIGHT - self.height
        ):
            return False
        return True
...
class Plane:
    ...
    def move(self):
        ...
        if self.position.getX() < -self.width:
            # make plane start at right side of screen
...
class Display(Frame):
    def __init__(self, root):
        ...
        self.frame = root
        self.canvas = Canvas(
            self.frame, width=CANVAS_WIDTH, height=CANVAS_HEIGHT, bg="white"
        )
        ...
        # create game objects
        self.plane = Plane(self.canvas, CANVAS_WIDTH, 0)
    ...
    def display_score(self):
        self.canvas.itemconfig(
            self.score_text,
            text="Level: " + str(self.level) + "  Score: " + str(self.score),
            font=self.scorefont,
        )
    ...
    def create_buildings(self):
        ...
        for building_num in range(0, CANVAS_WIDTH // SPACING):
            ...
            self.buildings.append(
                Building(self.canvas, building_num, height, self.building_width)
            )

    def drop_bomb(self):
        # only drop bomb if front of plane is in the screen
    ...
    def check_bomb(self):
        if not self.bomb.falling:
            return
        # did the bomb reach the bottom of the screen?
        ...
        for building in self.buildings:
            if building.is_inside(self.bomb.position):
                self.bomb.explode()
                building.shrink()
                # delete building if it has been destroyed
    ...
    def check_plane(self):
        ...
        for building in self.buildings:
            if (
                building.is_inside(plane_nose)
                or building.is_inside(plane_body_bottom)
                or building.is_inside(plane_wing)
            ):
                self.game_over()
        if (
            plane_body_bottom.getY() >= CANVAS_HEIGHT - 16
            and plane_body_bottom.getX() < 20
        ):
            self.plane_landed()
    ...
    def game_over(self):
        ...
        self.text = self.canvas.create_text(
            CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2, anchor="c", fill="black"
        )
    ...
    def plane_landed(self):
        ...
        self.text2 = self.canvas.create_text(
            CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 100, anchor="c", fill="black"
        )
        self.canvas.itemconfig(
